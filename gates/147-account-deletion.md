...
Branch cut from main at f709ebf (#144). **Code frozen at 57e8033.** This file
...
- `SidebarDrawerView.swift`: Logout alert copy only (Apple-backed accounts
  do restore).
- `wispTests/AccountDeletionTests.swift` (new): 8 tests.
...
warnings in any file the branch touches. The Studio has no iOS simulator
runtime installed, so the hermetic run happens on the Air.
...
is #4, #117 and #134, and nothing else fails. Expect **+8 tests** over main.
...
```
Prints nothing (checked at 57e8033).
